...
@section('body')
<form class="SignFlow" method="POST" action="{{ route('register') }}">
    @csrf
    <div class="SignFlow-account">
        <div class="SignFlowInput SignFlow-accountInputContainer">
            <div class="SignFlow-accountInput Input-wrapper">
                <input name="name" autocomplete="off" type="text" value="{{ old('name') }}" class="Input" placeholder="昵称">
            </div>
            <div class="SignFlowInput-errorMask {{$errors->has('name') ? '' : 'SignFlowInput-errorMask--hidden'}} SignFlowInput-requiredErrorMask">
    ...
    <div class="SignFlow-password">
        <div class="SignFlowInput">
            <div class="Input-wrapper">
                <input name="password_confirmation" type="password" class="Input" placeholder="确认密码">
            </div>
            <div class="SignFlowInput-errorMask {{$errors->has('password_confirmation') ? '' : 'SignFlowInput-errorMask--hidden'}} SignFlowInput-requiredErrorMask">
                {{ $errors->has('password_confirmation') ? $errors->first('password_confirmation') : '' }}
            </div>
        </div>
    </div>
    <button type="submit" class="Button SignFlow-submitButton Button--primary Button--blue">注册</button>
</form>
@endsection

@section('footer')
已有帐号？<a href="/login">登陆</a>
@endsection
