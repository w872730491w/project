...
@section('body')
<form class="SignFlow" method="POST" action="{{ route('password.request') }}">
    @csrf
    ...
    <div class="SignFlow-account">
        <div class="SignFlowInput SignFlow-accountInputContainer">
            <div class="SignFlow-accountInput Input-wrapper">
                <input id="email" type="email" class="Input" placeholder="邮箱" name="email" value="{{ $email ?? old('email') }}" required autofocus>
            </div>
